...
void Outline_float(float2 UV, float OutlineThickness, float DepthSensitivity, float NormalsSensitivity, float ColorSensitivity, float4 OutlineColor, float ThicknessFalloff, out float4 Out, out float OutDepthSample, out float OutEdge, out float4 OutOriginal)
{
    // scale down thickness further from the camera
    float ScaledThickness = OutlineThickness * SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, UV) * ThicknessFalloff; 
    if (ScaledThickness < OutlineThickness) {
        OutlineThickness = ScaledThickness;
    }
    ...
    OutDepthSample = SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, UV).r;
